...
namespace ElectroMod
{
    public class Report
    {
        ...
        public void GenerateReportMTO(Document doc, CenterCalculation calc)
        {
            ...
            AddFormula(doc, $"I_сз > k_н * (I_(к.з.max(K{calc.TransormatorMaxS.K})) * 1000) > 1.2 * ({calc.TransormatorMaxS.IkzMax} * 1000) > {calc.IkzMTO[3]}");
            AddParagraph(doc, "");
            ...
            if (KchuvMTO > 1.2)
            {
                ...
                if (calc.Bus.MTO > KchuvMTO)
                {
                    ...
                    else
                    {
                        ...
                        AddFormula(doc, $"I_сз=(I_(к.з.min(K1))*0,865*1000)/1,2 = ({Math.Round(Element.IkzMin, 3)}*0,865*1000)/1,2={NewMTO}");
            ...
            if (KchuvMTO > 1.2)
            {
                ...
                if (_recloser.MTO > KchuvMTO)
                {
                    ...
                    else
                    {
                        ...
                        AddFormula(doc, $"I_сз=(I_(к.з.min(K{Element.K}))*0,865*1000)/1,2 = ({Math.Round(Element.IkzMin, 3)}*0,865*1000)/1,2={NewMTO}");
                        AddParagraph(doc, $"Уставка МТО принимается {NewMTO}");
                    }
                }
                else
